...
VOICE_SYSTEM_SUFFIX = """
IMPORTANT: This response will be spoken out loud, not read. Keep it to 1-3 short sentences max.
No lists, no headers, no long explanations — just the direct answer, like a quick verbal reply.
"""


# --- Tier 3a: Gemini (default) ---
def call_gemini(query: str, context: str, voice_mode: bool = False) -> str:
    ...
    if voice_mode:
        system_prompt += VOICE_SYSTEM_SUFFIX

    max_tokens = 100 if voice_mode else 500
...
def call_anthropic(query: str, context: str, voice_mode: bool = False) -> str:
...
def route(query: str, context: str, voice_mode: bool = False) -> str:
    ...
    if PROVIDER == "anthropic":
        return call_anthropic(query, context, voice_mode)
    return call_gemini(query, context, voice_mode)
